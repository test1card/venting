# Venting v8.4 — 0D/Network model of depressurization (rigid volumes + compressible discharge)
...
**Политика версий:** в runtime активна только **v8.4**. Старый монолитный скрипт хранится только в `archive/` как архивный референс.
...
В v8.4 интегрируются `m` (и `T` в thermal-варианте), а давление вычисляется через EOS.
...
## 6) Архитектура кода (активная v8.4)
...
- `docs/model_v84.md` — краткая модель.
- `docs/verification_v84.md` — верификация и критерии.
- `docs/assumptions_v84.md` — допущения и ограничения.
