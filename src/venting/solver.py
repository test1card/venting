...
def solve_case_stream(
    ...
    """Streaming-compatible wrapper around solve_case.

    The default implementation preserves solver fidelity by computing the canonical
    batch solution and then emitting progressive chunks via callback.
    """
    ...
    for end in range(step, total + step, step):
        i_end = min(end, total)
        payload = {
            "t": sol.t[:i_end],
            "y": sol.y[:, :i_end],
            "progress": float(i_end / total),
            "done": bool(i_end >= total),
        }
        callback(payload)
        if i_end >= total:
            break
